...
def commitJob(userName, db, title, description, employer, location, salary):
    ...
    if (count > 10):
        ...
        db[0].execute("INSERT INTO jobs VALUES (?,?,?,?,?,?,?)",(userName, title,description, employer, location, salary, time.time()))
        db[1].commit()
    ...
    if choice >= 1 and choice <= 10:
        ...
        if option == 1:
            ...
            db[0].execute("INSERT INTO appliedFor VALUES (?,?,?,?,?,?,?,?)",
                         (username, jobs[0][choice - 1][3], jobs[0][choice - 1][1], graduateDate, startDate, reason, whyThis, time.time()))
            db[1].commit()
