...
#hasapplied recently and isnewjobs are commented out for the moment but need to be re-implemented
# commit a job
def commitJob(userName, db, title, description, employer, location, salary):
    ...
    if (count > 10):
        ...
        db[0].execute("INSERT INTO jobs VALUES (?,?,?,?,?,?,?)",
                      (userName, title, description, employer, location, salary, dt_object))
        db[1].commit()
        API.printJobs(db)
...
def printJobs(db, username):
    ...
    choice = input('\nEnter corresponding job number or 11 to exit: ')
    choice = int(choice)
    if choice == 11:
        return

    elif (choice > 0 and choice < 11):
        option = input("Enter 1 to apply for job, 2 to save job, or 3 to exit: ")
        option = int(option)
        # apply for job
        if (option == 1):
            ...
            db[0].execute("INSERT INTO appliedFor VALUES (?,?,?,?,?,?,?,?)",
                          (username, jobs[0][choice - 1][3], jobs[0][choice - 1][1], graduateDate, startDate, reason,
                           whyThis, dt_object))
            db[1].commit()
        ...
        # save job
            ...
            db[0].execute("INSERT INTO savedJobs VALUES (?,?,?)",
                          (username, jobs[0][choice - 1][3], jobs[0][choice - 1][1]))
            db[1].commit()
...
def jobSearch(db, isLoggedIn, username):
    ...
    if (isLoggedIn == 0):
        ...
        while (y != 7):
            ...
            # user option
            y = input("Choose an available option, or enter 7 to exit: ")
            y = int(y)
            # print all jobs
            if (y == 1):
